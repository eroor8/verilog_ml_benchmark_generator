    ...
    def construct(s, mlb_spec={}, wb_spec={}, ib_spec={}, ob_spec={}, emif_spec={},
        ...
        s.load_wbufs_emif = SM_LoadBufsEMIF(buffer_counts['W'], 2**addrw_ports[0]["width"],
                                   ...
                                   utils.get_sum_datatype_width(emif_spec,
                                                                "AVALON_WRITEDATA", "in"),
                                   w_address)
        utils.connect_out_to_top(s, s.load_wbufs_emif.buf_writedata, "wbuf_writedata")
        wen_ports = list(utils.get_ports_of_type(buffer_specs['W'], 'WEN', ["in"]))
        ...
        s.load_ibufs_emif = SM_LoadBufsEMIF(
            ...
            utils.get_sum_datatype_width(emif_spec, "AVALON_WRITEDATA", "in"),
            i_address)
        utils.connect_out_to_top(s, s.load_ibufs_emif.buf_writedata, "ibuf_writedata")
        ...
        if (ws):
            utils.connect_out_to_top(s, s.preload_weights.wen, "stream_weights_wen")
        else:  
            utils.connect_out_to_top(s, s.stream_weights.wen, "stream_weights_wen")  
        s.pstart_address_wide = Wire(max(int(math.log(ubt*ubt+1,2)),addrw_ports[0]["width"])+addri_ports[0]["width"]+1)
        s.preload_weights.start_address //= s.pstart_address_wide[0:addrw_ports[0]["width"]]
        ...
        s.emif_waitrequest = InPort(1)
        s.emif_readdatavalid = InPort(1)
        s.emif_writedata = OutPort(utils.get_sum_datatype_width(emif_spec,
                                                                "AVALON_WRITEDATA", "in"))
...
class MultipleLayerSystem(Component):
    def construct(s, mlb_spec={}, wb_spec={}, ib_spec={}, ob_spec={}, emif_spec={},
        ...
        connected_ins += utils.mux_ports_by_name(s,statemachines, "emif_address", s.emif_inst,
            "address", insel=s.sel)
        connected_ins += utils.mux_ports_by_name(s,statemachines, "emif_writedata", s.emif_inst,
            "writedata", insel=s.sel)
        connected_ins += utils.mux_ports_by_name(s,statemachines, "emif_read", s.emif_inst,
            "read", insel=s.sel)
        connected_ins += utils.mux_ports_by_name(s,statemachines, "emif_write", s.emif_inst,
            "write", insel=s.sel)
